...
def wavelength_from_index(index):
    index2 = index + 2
    return A0 +B1*index2 +B2*index2**2 +B3*index2**3 +B4*index2**4 +B5*index2**5
...
if plot_in_bkg:
    ...
    print(" - Press p to store current frame in memory ")
    print(" - Press s to save and close")
...
def plot_bkg_check():
    ...
    if select.select([sys.stdin], [], [], 0)[0]:
        ...
        if command == 's':
            print("\nSaving data")
            np.save("output_array.npy", ytot)
            np.save("integrate_info.npy", integrate_info_tot)
            ser.close()
            return False
        ...
        if command == 'p':
            yp = y.copy()
            yp = yp.reshape(-1, 1)  # make y a column vector
            integrate_infop = integrate_info.copy()
            integrate_infop = integrate_infop.reshape(-1, 1)

            if ytot is None:
                ytot = yp  # first column
                integrate_info_tot = integrate_infop
            else:
                ytot = np.concatenate((ytot, yp), axis=1)
                integrate_info_tot = np.concatenate((integrate_info_tot, integrate_infop), axis=1)
            print(integrate_info_tot.shape)
            print(ytot.shape)
